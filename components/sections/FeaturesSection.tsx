...
export function FeaturesSection({ loaded, mx, my, heroOp, heroY }: { loaded: boolean; mx?: MotionValue<number>; my?: MotionValue<number>; heroOp?: MotionValue<number>; heroY?: MotionValue<number> }) {
  return (
    <>
      {[
        { icon: MatrixStreamIcon, title: 'Matrix Stream', sub: 'Content-Addressed Associative Memory', body: 'A 384×384 grid storing key-value pairs — 147,456 independent dimensions. It writes corrections via the delta rule: check what memory knows, compute the error, stamp only the difference. Like a librarian who fixes wrong answers without rewriting the encyclopedia.', color: C.blue },
        { icon: VectorStreamIcon, title: 'Vector Stream', sub: 'Selective Sequential Recurrence', body: 'A 384-element vector tracking recent patterns with Mamba-style selective gating. It decides per-token how much to remember and how much to forget — like a rolling average that intelligently adapts its sensitivity.', color: C.purple },
